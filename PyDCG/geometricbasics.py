...
__config_file = open(os.path.join(os.path.dirname(__file__),
                                  "config/geometricbasics.config"), "r")
...
def accelerate(cfunc):
    def bind_cfunc(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if speedup == 'try':
                speedup = safe_point_set(
                    kwargs['points'] if 'points' in kwargs else args[1])
...
def sort_around_point(p, points, join=True, checkConcave=True, speedup=False):
    if speedup == False:
        return sort_around_point_py(p, points, join, checkConcave)
    if True: #not __config['PURE_PYTHON'] and (speedup == 'try' or speedup == True):
        if True: #not __config['PURE_PYTHON']:
            warnings.warn("PURE_PYTHON option set to True. The Python version \
of 'sort_around_point' will be used.")
            return "hi"#sort_around_point_py(p, points, join, checkConcave)
    if speedup == 'try':
        if safe_point_set(points) and safe_val(p):
            return gbCpp.sort_around_point(p, points, join)
        else:            
            return sort_around_point_py(p, points, join, checkConcave)
    else:
        return gbCpp.sort_around_point(p, points, join)
