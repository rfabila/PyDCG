...
def count_non_convex_four_islands(pts, colored=False):
    ...
    for p in pts:
        ...
        non_convex = non_convex + \
            count_non_convex_four_islands_p(p, tpts, colored=colored)
    return non_convex
...
def report_empty_triangles_p(p, points, speedup=True):
    ...
    try:
        return holesCpp.report_empty_triangles_p(p, points)
    except OverflowError:
        return report_empty_triangles_p_py(p, points)
...
def slow_count_empty_triangles_for_each_p(points):
    ...
    for i in range(len(points)):
        ...
        points[0] = pt
        triangs = triangs + \
            slow_count_empty_triangles_around_p(points[0], points[1:])
    return triangs
    ...
    for i in range(len(sorted_points)):
        if turn(p, sorted_points[i],
            ...
            for j in range(len(sorted_points)):
                new_sorted_points[j] = sorted_points[(
                    i+1+j) % len(sorted_points)]
...
def count_convex_rholes_py(points, r, mono=False):
    ...
    for p in range(len(points)):
        ...
        for q in range(len(right_points)-1):
            ...
            idx.sort(key=cmp_to_key(
                lambda x, y: L[(q, outgoing_vertices[y])] - L[q, outgoing_vertices[x]]))
            # l.sort(key=cmp_to_key(lambda v1, v2: turn(p, v1, v2)))
            ...
            for vo in outgoing_vertices:
                if L[(q, vo)] >= r-2:
                    if mono:
                        if (right_points[q][2] == color and
                                right_points[vo][2] == color):
                            tmplist = [right_points[vo],
                                       right_points[q], points[p]]
                        else:
                            C[(q, vo)] = []
                    else:
                        tmplist = [right_points[vo],
...
def countEmptyMonoTriangs(points):
    ...
    for i in range(0, len(points)):
        triangs = triangs + \
            countEmptyMonoTriangsVertex(ordpoints[i][1], points[i][2])
...
def count_deg_triang(points):
    ...
    for i in range(0, len(points)):
        triangs = triangs + \
            count_deg_triang_vertex(ordpoints[i][1], degs, i, points)
    for i in range(0, len(points)):
