...
class Vis:
    ...
    def __init__(self, h=500, w=500, points=[], lines=[], segments=[], center=None,
        ...
        if pic_button:
            photocam = PhotoImage(master=self.paper, file=os.path.join(
                os.path.dirname(__file__), "Icons/camera.gif"))
        ...
        if move_points_button:
            hand = PhotoImage(file=os.path.join(
                os.path.dirname(__file__), "Icons/hand.gif"))
            boton = Button(frame, image=hand,
                           command=self.change_state("move point"))
            boton.pack(side=LEFT)
            boton.image = hand

        if add_point_button:
            add_pic = PhotoImage(file=os.path.join(
                os.path.dirname(__file__), "Icons/add.png"))
            boton = Button(frame, image=add_pic,
                           command=self.change_state("add point"))
            boton.pack(side=LEFT)
            boton.image = add_pic

        if delete_point_button:
            del_pic = PhotoImage(file=os.path.join(
                os.path.dirname(__file__), "Icons/gtk-remove.png"))
            boton = Button(frame, image=del_pic,
                           command=self.change_state("delete point"))
            boton.pack(side=LEFT)
            boton.image = del_pic
    ...
    def closest_point(self, point):
        ...
        for p in range(len(self.points)):
            new_min = (self.points[p][0]-point[0])**2 + \
                (self.points[p][1]-point[1])**2
            if new_min < min:
                min = new_min
                q = p
        return q
    ...
    def draw(self):
        ...
        if self.update_objects is not None:
    ...
    def drawPolygons(self):
        ...
        for P in self.polygons:
            if P.bounded:
                vertices = [self.convert_to_screen_coords(
                    p) for p in P.vertices]
                Q = self.paper.create_polygon(
                    vertices, fill=P.fill, outline=P.outline)
                ...
                # translation
                lu_corner = [lu_corner[0]+self.center[0],
                             lu_corner[1]+self.center[1]]
                ld_corner = [ld_corner[0]+self.center[0],
                             ld_corner[1]+self.center[1]]
                ru_corner = [ru_corner[0]+self.center[0],
                             ru_corner[1]+self.center[1]]
                rd_corner = [rd_corner[0]+self.center[0],
                             rd_corner[1]+self.center[1]]
    ...
    def draw_grid(self):
        ...
        for s in grid_segments:
            ...
            self.drawnsegments.append(self.paper.create_line(
                p[0], p[1], q[0], q[1], fill=self.grid_color))
    ...
    def drawLines(self):
        ...
        for l in self.lines:
            ...
            if draw:
                ...
                self.drawnlines.append(self.paper.create_line(
                    p[0], p[1], q[0], q[1], fill=l.color))
        ...
        for p in self.points:
            ...
            if len(p) <= 2:
                self.drawnpoints.append(self.paper.create_oval(
                    x-self.t, y-self.t, x+self.t, y+self.t, fill="black"))
            else:
    ...
    def leftclick(self, event):
        if self.state == "view":
            ...
            distance = (point[0]-self.points[q][0])**2 + \
                (point[1]-self.points[q][1])**2
