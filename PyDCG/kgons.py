...
import itertools
from . import geometricbasics, convexhull
...
def maxKgon(pts):  # Finds the largest rgon in pts in time O(n^3))
    ...
    # Finds the point that makes the smallest ccw angle with line pts[i]pts[j], measured around pts[j]
    def findNext(i, j):
        if i == j:
            return -1
    ...
    nextpt = [[findNext(i, j) for j in range(len(pts))]
...
def maxKgonN4(pts):  # Finds the largest rgon in pts in time O(n^4))
    ...
    # Finds the largest rgon with q as left-most point and pts[i], pts[j] as last vertices, when sorted ccw beginning at q
    def step(q, i, j):
        if tab[i][j] != -1:  # already know the max
            return tab[i][j]
        for k in range(len(pts)):  # try to extend it with every point in pts
        ...
        for j in range(len(pts)):
            ...
            for k in range(len(pts)):
                ...
                if s < q or geometricbasics.turn(q, r, s) >= 0:
                    # both r and s are to the right of q and (q, r, s) form a left turn
                    continue
                maxH = max(step(q, j, k), maxH)
    return maxH


# Super naive function. Tries every combination of n points (starting with n=3) and checks if they are in convex position.
def naiveMaxKgon(pts):
    maxH = 0
    s = []
