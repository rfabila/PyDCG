...
class rational(object):
    ...
    # TODO: Check if this is necessary (I think so, since somewhere I use rationals as keys but I'm not sure anymore)
    def __hash__(self):
        return tuple.__hash__((self.a, self.b))
        # auxa = 2 * self.a if self.a > 0 else -2 * self.a - 1
        # auxb = 2 * self.b if self.b > 0 else -2 * self.b - 1
        # return ((auxa + auxb) * (auxa + auxb + 1) // 2) + auxb

    def toDecimal(self):
        return decimal.Decimal(self.a)/decimal.Decimal(self.b)
...
def getPointRegion(q, sortedpoints, indices):
    ...
    ############
    # print(upper, lower)
    # return upper, lower  # list(map(dualize, upper)), list(map(dualize, lower))
...
def getRegionR(upper, lower, toList=True):
    ...
    while not (doneU and doneL):
        ...
        if idxU == len(U) - 1 or turn(l[0], l[1], U[idxU + 1][0]) > 0:
            ...
            if (U[idxU + 1][0][0] < U[idxU][0][0] < L[idxL][0][0]) or \
                    (U[idxU + 1][0][0] > U[idxU][0][0] > L[idxL][0][0]):
                doneU = True
            else:
                doneU = False
        else:
...
def randPointPolygon(poly, tries=100):
    ...
    for i in range(len(weights)):
        try:
            ...
            # a = decimal.Decimal(weights[i].a)
            # b = decimal.Decimal(weights[i].b)
            weights[i] = weights[i].toDecimal()
