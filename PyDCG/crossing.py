...
def count_crossings_candidate_list_py(point_index, candidate_list, pts):
    ...
    def gen_antipodal_list(q, pts_q):
        ...
        intervals_q = geometricbasics.sort_around_point(
            q, intervals_q, speedup=False)
        return intervals_q
    ...
    def remove_j_sortandmark(i, pts, sortandmark=True):
        ...
        if sortandmark:
            q = pts[i]
            tmp_pts = geometricbasics.sort_around_point(
                q, tmp_pts, speedup=False)
    ...
    def join_pts_antipodal_candidatelist(point_index, pts, central_point, tmp_pts, candidate_list):
        ...
        united_pts = geometricbasics.sort_around_point(
            central_point, united_pts, speedup=False)
        return united_pts
    ...
    def change_of_cr_for_list(united_pts, candidate_list, position_p, nis):
        ...
        for k in range(1, len(united_pts)):
            ...
            if united_pts[pos][3]:
                ...
                change_cr_list[united_pts[pos][2]
                               ] = change_cr_list[united_pts[pos][2]]+sum_ni
    ...
    for i in index_pts_without_pointindex(point_index, pts):
        ...
        #
        united_pts = join_pts_antipodal_candidatelist(
            point_index, pts, pts[i], tmp_pts, candidate_list)
        position_p = searchpoint(pts[point_index], united_pts)
        [aux_cr_list2, count_change_of_list] = change_of_cr_for_list(
            united_pts, candidate_list, position_p, nis)
        ...
        for j in range(len(candidate_list)):
            cr_list3[j] = cr_list3[j]+(count_change_of_list[j]+nis[pos_p_in_tps])*(
                count_change_of_list[j]+nis[pos_p_in_tps]-1)//2
        # ----------------------
